#!/usr/bin/env python3
"""
Run two complete ThereminMocap training cycles over all discovered MOCAP CSV stimuli.

Cycles:
    frame : per-frame model, seq_len = 1
    seq5  : sequence model, seq_len = 5

MOCAP input files are read from --mocap-dir, which defaults to:

    <feature-dir>/mocap

Expected MOCAP CSV files:

    <stimulus>.csv
    <stimulus>_cleaned.csv

Expected audio target files remain in --feature-dir:

    <stimulus>_audio.npy or <stimulus>_audio.csv

The MOCAP CSV is expected to contain:

    Frame
    antenna markers, for example pitch_001_X/Y/Z, pitch_002_X/Y/Z, pitch_003_X/Y/Z
    hand markers, for example right_001_X/Y/Z ... right_006_X/Y/Z
    optional camera / ZED marker columns

Feature construction:
    1. Detect pitch or volume from the stimulus name.
    2. Extract hand markers:
           pitch  -> first 6 detected hand markers by default
           volume -> first 3 detected hand markers by default
    3. Extract the three antenna markers for the corresponding category.
    4. Compute the antenna center of mass as the per-frame mean of the three
       antenna markers.
    5. Express hand coordinates relative to that antenna center.
    6. Rescale the relative hand coordinates by the antenna size, defined as
       the maximum pairwise distance between the three antenna markers.
    7. Normalize coordinate ranges using all pitch files together and all
       volume files together, separately.

The script is standalone for the MOCAP CSV workflow. It does not import or
use dataset.py. It uses HandNet from network.py.
"""
...
DEFAULT_OUTPUT_DIR = "/home/mmlab/Desktop/Theremin/ThereminMocap/runs_mocap_multi_cv_dual"
...
def discover_mocap_stimuli(
    ...
    mocap_paths = sorted(glob.glob(os.path.join(mocap_dir, "*.csv")))

    if not mocap_paths:
        raise RuntimeError(f"No MOCAP CSV files found in: {mocap_dir}")
    ...
    if not pairs:
        raise RuntimeError(
            f"No valid MOCAP/audio pairs found. MOCAP dir: {mocap_dir}, "
