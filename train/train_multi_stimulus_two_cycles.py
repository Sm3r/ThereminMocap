#!/usr/bin/env python3
"""
Run two complete ThereminMocap training cycles over all discovered stimuli.

Cycles:
    frame : per-frame model, seq_len = 1
    seq5  : sequence model, seq_len = 5
...
DEFAULT_OUTPUT_DIR = "/home/mmlab/Desktop/Theremin/ThereminMocap/runs_multi_cv_dual"
...
def discover_stimuli(feature_dir: str, target_suffix: str, target_ext: str) -> List[Dict[str, str]]:
...
def clean_hand_array(hand_arr: np.ndarray, max_nils_to_fill: int) -> Tuple[np.ndarray, np.ndarray]:
    if hand_arr.ndim != 2:
...
def save_data_overview_plots(x_arr: np.ndarray, y_arr: np.ndarray, out_dir: str) -> None:
    ...
    for feature_idx in feature_indices:
        ...
        ax.plot(x_feat, label=f"hand feature {feature_idx} normalized", alpha=0.7)

    ax.set_title("Target vs selected hand features")
    ax.set_xlabel("Aligned hand-frame index")
    ax.grid(True, alpha=0.3)
    ax.legend(loc="best")
    fig.tight_layout()
...
def train_one_fold(
    ...
    print()
    print("=" * 80)
    print(f"Cycle: {cycle_name} | seq_len={seq_len} | Stimulus: {stimulus} | Fold {fold_idx + 1}/{args.n_folds}")
    print("=" * 80)
    ...
    train_ds = NpySequenceDataset(x_arr, y_arr, frames, train_ends, seq_len)
    test_ds = NpySequenceDataset(x_arr, y_arr, frames, test_ends, seq_len)
    ...
    if val_ends is not None:
        val_ds = NpySequenceDataset(x_arr, y_arr, frames, val_ends, seq_len)
...
def parse_args():
    ...
    parser.add_argument("--target-ext", type=str, default="npy", choices=["npy", ".npy", "csv", ".csv"])
...
def main() -> None:
    ...
    pairs = discover_stimuli(args.feature_dir, args.target_suffix, args.target_ext)
    ...
    for pair in pairs:
        ...
        print()
        print("#" * 80)
        print(f"Loading stimulus: {stimulus}")
        print("#" * 80)

        hand_arr = load_array(pair["hand_path"], csv_target_column=args.csv_target_column)
        target_arr = load_array(pair["target_path"], csv_target_column=args.csv_target_column)
    ...
    if args.fold_results_csv is None:
        args.fold_results_csv = os.path.join(args.output_dir, "crossval_fold_results.csv")
    write_csv(args.fold_results_csv, all_fold_rows, fold_fieldnames)

    summary_rows = summarize_fold_rows(all_fold_rows)
    summary_fieldnames = list(summary_rows[0].keys()) if summary_rows else ["cycle", "seq_len", "stimulus", "n_folds"]

    if args.summary_csv is None:
        args.summary_csv = os.path.join(args.output_dir, "crossval_summary.csv")
    ...
    for row in summary_rows:
        print(
            f"{row['cycle']} | seq_len={row['seq_len']} | {row['stimulus']} | "
